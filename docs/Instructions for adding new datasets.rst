...
The following is a tentative list of details that should be mentioned while entering data in the HybriD3 database, along with the probable location in published literature where such information can be found. This document can be used as a general guideline for the purpose of adding new datasets.
...
2.	Space group (search for “space group”) [If there is a suffix in the notation, type it in the first bracket. e.g., for Pnm21, type it as -  Pnm2(1)]
...
As always, please include the original reference to the authors' work (typically, a journal reference) in the HybriD3 database along with the data for proper reproducibility, attribution, and further details.
...
E.g., lead iodide (PbI2, 99.99%, Sigma-Aldrich), methylammonium iodide (MAI, >99%, Alfa Aesar), etc.
For “Product,”
<color><shape><formula><sample type>
E.g., Orange plate-like (PEA)2PbI4 crystals
...
As before, please do not copy non-trivial segments of text literally from other sources but rather include just the bare data in the format consistent with other entries in the HybriD3 database itself.
